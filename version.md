...
- 优化归档交互：归档成功后清空当前提示词输入框并移除本地草稿缓存，方便直接继续输入下一条
- 归档后自动聚焦输入框，并同步更新字符计数与状态提示

### v0.1.64 — 2026-09-22

- 在线提示词页面响应增加 no-store 缓存策略，避免浏览器继续使用旧版页面导致归档后输入框未清空
- 归档清空前先取消草稿自动保存防抖任务，防止旧草稿在清空后被异步写回

### v0.1.65 — 2026-09-23

- 将 Hub 顶层品牌从 LlamaManager 更新为 HannisHub
...
- 补充模型下载页面的初始加载状态，统一覆盖 Loading、Error 与 Empty 三类反馈

### v0.1.68 — 2026-09-23

- 新增「点云查看器」统一前端模块，可配置多个本机顶层目录并递归发现 PLY、PCD、XYZ、PTS、LAS、LAZ 等点云文件
- 扫描结果自动跨过 `point_cloud`、`iteration_*`、`model`、`world` 等通用层级，优先按实验或结果文件夹名称归并显示
- 新增受限点云目录配置、数据集扫描和原始文件读取 API；文件读取仅允许访问已配置顶层目录内部的相对路径，并防止路径越界
- 前端接入开源 Three.js 的 PLY/PCD 加载器与轨道控制，支持点云旋转、缩放、平移、颜色保留和点大小调节
- `~/reproduce` 存在时会作为首次打开点云查看器的默认扫描目录；已验证识别其中 839 个 PLY 文件并归并为 231 个结果目录
- 同步更新统一导航、总览服务入口、架构文档与前端依赖

### v0.1.69 — 2026-09-23

- 进入具体子模块时默认收起桌面端左侧导航，仅保留图标栏以扩大内容区域
- 总览页默认展开导航，顶部按钮支持在展开与收起状态之间切换
- 移动端继续使用抽屉式导航，并为收起状态的导航图标补充悬浮提示

### v0.1.70 — 2026-09-23

- 压缩提示词工作区的页头、操作控件、归档卡片与列表间距，减少辅助内容占用
- 将提示词主编辑窗口扩大为自适应 480–760px 高度，移动端同步增加可用编辑区域

### v0.1.71 — 2026-09-23

- 提示词页仅保留「提示词 / Workspace」标识，移除页面主标题、说明文字与刷新归档按钮
- 移除当前提示词卡片中关于本地润色排版的说明文字

### v0.1.72 — 2026-09-23

- 将提示词页顶部留白从 64px 收紧到 32px，让「提示词 / Workspace」标识更贴近顶栏
- 归档列表条目改为单行提示词摘要，超出部分省略号截断
- 归档条目的日期与字符数移动到摘要上方，形成“元信息 + 内容摘要”两级结构

### v0.1.73 — 2026-09-23

- 修复提示词字符数变化时挤压中部「归档到」分组选择器的问题
- 编辑器底部元信息改为固定三列网格布局，字符数、分组选择和自动暂存位置保持稳定
- 字符数启用等宽数字显示，移动端仍保持纵向排列

### v0.2.0 — 2026-09-23

- 新增文件管理组件，默认仅暴露 `~/reproduce`，支持受限目录浏览、服务端目录缓存、手动同步与文件流式下载
- 文件浏览页提供顶层目录切换、面包屑导航、当前目录搜索、“仅点云”过滤、文件大小与修改时间展示，以及单个文件下载
- 点云查看器归入文件管理模块，扫描范围改为从已暴露顶层目录中选择，并可通过搜索选择子文件夹；保留 `/point-clouds` 兼容入口
- 点云扫描复用文件管理目录能力与缓存，新增 15 秒扫描结果缓存，重复打开显著提速；可强制重新扫描
- 点云预览工具栏新增当前文件下载按钮，便于直接下载 PLY 等文件
- 新增文件管理设置、目录读取、文件夹选项、缓存同步与文件下载 API，并同步更新架构文档与使用说明

### v0.2.1 — 2026-09-23

- 提示词工作区的「归档」按钮升级为「归档并复制」
- 点击归档时先把当前原文或润色稿写入剪贴板，再执行归档和清空编辑器
- 归档与复制共用剪贴板逻辑；复制失败时仍保留归档结果，并在提示中说明复制失败

### v0.2.2 — 2026-09-23

- 新增统一「AI 能力设置」模块，提供 OpenAI 兼容 API 地址、模型名称、API 密钥和音频转写提炼提示词配置
- 新增 `/settings` 前端页面与 `/api/ai-settings` 读取、保存、测试接口；密钥只在本机保存并永不回显
- AI 配置保存到根目录 `ai_settings.json`，该文件及其临时写入文件均已加入 `.gitignore`
- 首次读取时自动从旧版模型管理配置迁移 AI API、模型、密钥和 ASR 提炼提示词
- ASR 提炼改用统一 AI 配置；ASR 页面移除提示词编辑，模型设置页移除 AI 接口配置
- 保留旧版 ASR 提炼提示词接口与 OpenAI 测试接口，内部改为读写统一配置

### v0.2.3 — 2026-09-23

- ASR 提炼提示词设置移回音频转写页面，AI 设置页仅统一管理 API 地址、模型与密钥
- AI 设置页新增“测试链接”“探查模型列表”“测试模型”三个操作
- 模型输入框关联探查到的模型下拉列表，同时保留自定义输入能力
- 新增模型列表探查与单模型最小对话测试 API
- ASR 页面展示当前提炼模型，并可直接保存提炼提示词

### v0.2.4 — 2026-09-23

- 提示词「复制」改为纯前端即时操作：不再等待 Clipboard Promise 完成后才提示，也不触发任何后端请求
- 「归档并复制」点击后立即复制并显示「已复制，正在归档…」，归档请求在后台继续执行
- 归档请求与剪贴板写入并行，归档完成后异步刷新列表，不再阻塞复制反馈和编辑器即时状态
- 归档进行中锁定重复点击、编辑器、模式切换与分组选择，避免慢请求期间状态错乱或重复归档
- 归档失败时保留错误提示和编辑器内容；复制失败与归档失败会分别给出明确反馈
- 非安全上下文缺少异步 Clipboard API 时，使用浏览器同步复制兜底

### v0.2.5 — 2026-09-24

- 修复点云查看器读取子文件夹列表缺少函数导入、扫描接口参数传递错误导致的 500 请求失败
- 点云页先读取顶层目录配置；未配置目录时给出可操作提示，子文件夹列表失败时仍可扫描点云
- 发现的点云结果按项目分组，文件列表显示结果目录内的相对路径；扫描范围默认优先列出有点云结果的文件夹，仍可搜索全部目录
- 扫描期间点云文件被移动或删除时跳过该文件，避免整个扫描请求失败

### v0.2.6 — 2026-09-24

- 文件浏览页集成点云预览，在当前目录直接选择 PLY、PCD、XYZ、PTS 等文件查看，无需扫描整棵目录树
- 将 Three.js 预览逻辑拆为可复用组件，保留点大小调整、交互视角与文件下载
- 移除独立点云扫描界面、扫描接口及相关缓存；旧点云页面入口展示统一文件浏览界面，旧文件读取地址保留兼容

### v0.2.7 — 2026-09-24

- 文件浏览页移除暴露范围编辑窗口，默认直达指定的 RadioGS 点云目录并自动预览 `point_cloud.ply`
- 目录浏览增加浏览器端短期缓存，减少返回已访问目录时的等待
- 扩大点云画布与页面可用宽度，新增大屏预览按钮，并压缩页头和工具栏占用
- Gaussian PLY 读取颜色、透明度和尺度后使用二维画布绘制；无 WebGL 时也能显示点云
- 点云文件读取时在画布显示下载百分比和解析状态，避免加载期间出现无提示空白

### v0.2.8 — 2026-09-24

- 文件浏览区增加明确的“返回上级”按钮，切换目录时清空当前目录搜索词
- 移除文件列表与点云预览工具栏中的下载按钮
- 点大小默认设为 1.0，滑块下限降至 0.2；Gaussian 预览改用小方点绘制，减少涂抹感
- 增加文件颜色与主题单色切换开关，切换时直接更新预览

### v0.2.9 — 2026-09-24

- 将目录浏览区移到点云预览下方，预览与目录均占满页面内容宽度
- 文件夹和文件条目改为单行布局，文件名、大小与修改时间横向排列；窄屏隐藏次要元信息以保持单行
- 顶层目录选择、当前目录搜索和“仅点云”过滤调整为横向工具栏

### v0.2.10 — 2026-09-24

- 文件管理导航拆为“文件游览”和“点云预览”两个入口；文件游览页选择点云时带路径进入预览页
- 点云页移除页面和预览卡片标题，预览画布使用固定高度，浏览目录时保留当前点云
- 在点云预览与当前目录之间增加目录收藏，支持收藏当前目录、快速跳转、修改显示名称和移除
- 收藏记录保存到 `settings.json`，新增收藏接口并校验目录仍属于已开放范围
- 目录浏览补充符号链接越界校验，避免通过收藏或直接请求跳出开放目录
- Gaussian 点云不再创建多余的 WebGL 画布，确保二维预览层直接可见

### v0.2.11 — 2026-09-24

- 修复短窗口下主导航侧栏无法滚动的问题，底部导航不再被视口截断
- AI 能力设置新增“可支持的模型”按钮区，模型探查后展示全部模型 ID
- 点击模型按钮会自动填入模型选择，同时保留自定义模型名称输入和原有 datalist 提示

### v0.2.12 — 2026-09-24

- 左侧主导航改为可手动展开与收起：收起后是 72px 宽的图标小格列，分组之间用分隔线区分，鼠标悬停显示名称
- 侧栏顶部新增面板样式的展开/收起按钮，收起状态写入浏览器 localStorage，刷新和切换页面后保持同一形态
- 取消按页面路径自动收起侧栏的旧逻辑；移动端侧栏仍固定为抽屉形态，顶栏按钮仅在窄屏显示
- 修复导航条目超出视口高度时覆盖侧栏底部信息的问题，侧栏整体恢复滚动
- 浏览器标签标题跟随当前模块变化，例如进入点云预览显示「点云预览 · HannisHub」、进入提示词显示「提示词 · HannisHub」；登录页与 404 页同步设置标题

### v0.2.13 — 2026-09-25

- 点云预览页的目录列表为每个点云文件（PLY 以及 PCD、XYZ、XYZN、XYZRGB、PTS、LAS、LAZ）增加独立下载按钮，直接复用受限目录下载接口
- 下载按钮排在行内操作末尾并带文件名提示；LAS/LAZ 仍先标注“暂不支持预览”再提供下载
- 下载入口只在点云预览页出现，文件游览页保持原有预览入口，预览画布本身不显示下载按钮

### v0.2.14 — 2026-09-25

- 目录缓存有效期延长至一小时；“同步目录”会递归预读所选项目的目录树，默认同步 RadioGS-perlight 和 RadioGS-stage1，并显示缓存目录数
- 点云预览顶部展示当前文件的完整路径，以及该文件所属收藏目录的自定义名称
- 收藏目录支持从当前目录直接进入改名；收藏记录按真实目录匹配并兼容顶层目录路径写法变化

### v0.2.15 — 2026-09-25

- 点云预览页移除当前目录工具栏中的顶层目录选择、搜索框和“仅点云”开关，保留目录同步操作
- 文件游览页继续提供原有目录筛选控件

### v0.2.16 — 2026-09-25

- 点云预览目录列表递归显示当前路径下的全部 PLY 文件，文件名后标注相对路径并将文件行缩进一级
- 文件夹保留顶格显示，其他普通文件从点云目录列表隐藏；文件游览页的直接子项浏览不变
- 新增受限目录递归 PLY 接口，并让“同步目录”同时预热 PLY 列表缓存

### v0.2.17 — 2026-09-25

- 点云页保留当前文件夹递归探查，并增加跨多个项目目录的按日期查找 PLY；默认预填已开放范围内的 RadioGS 和 LumiMotion 目录
- 日期按实验目录名称开头匹配，默认只显示 `iteration_40000` 等第 40000 次迭代文件；可修改迭代次数或留空查全部
- 探查路径保存到 `settings.json`，受已开放目录边界校验；结果显示完整文件路径并可直接预览和下载

### v0.2.18 — 2026-09-25

- 点云页将日期查找移到顶部，默认自动显示含今天在内的近 7 天结果，并提供“今天”和自定义日期范围；手动查找重新扫描磁盘以发现新文件
- 迭代过滤改为“每组最新迭代”“指定迭代”“全部 PLY”三种方式，默认按各点云输出组选择最大迭代数，兼容不同项目的迭代上限
- 不再默认预览旧的固定日期 PLY；文件夹探查保留在页面最后，日期结果继续显示完整路径并支持预览、下载

### v0.2.19 — 2026-09-26

- 新增红黑配色的 HannisHub 浏览器标签页图标，统一 Next.js 页面与迁移期保留的旧页面全部引用
- 提示词工作区新增轻度、标准、深度三档 AI 润色，复用项目设置中的 OpenAI 兼容 API、模型与密钥，并完成真实模型调用验证
- 在提示词页面底部新增三档润色指令设置，支持分别编辑、保存及恢复内置默认值
- 归档列表取消前端分组操作，改为按时间倒序的一体列表；固定列表高度并独立滚动，继续保留最多 500 条的自动淘汰保护
- 新增润色执行与润色设置 API，并在 spec.md 中同步接口、配置字段和限制

### v0.2.20 — 2026-09-26

- 点云预览改为页面顶部的左右 A/B 双画布，可同时载入两个文件并分别旋转、缩放和平移进行对比
- 新增预览载入目标选择、左右交换、单侧清空与单侧大屏操作，日期结果和文件夹列表会明确显示当前载入目标
- 按日期查找 PLY 移到双栏预览下方；窄屏时双画布自动改为上下排列

### v0.2.21 — 2026-09-26

- 将轻度、标准、深度三个润色按钮移动到编辑器顶部，排在原文/润色稿切换左侧，并精简为短标签
- 参考 OpenAI 与 Anthropic 官方提示工程原则重新设计三档默认润色指令，分别控制最小修改、清晰结构与深度任务重构
- 强化原始提示词的内容边界，保留事实、约束、示例、变量、路径和代码，避免润色过程回答原始任务或臆造信息
- 重启后台服务加载新增润色接口，修复新前端请求旧后端时出现的 Not Found

### v0.2.22 — 2026-09-26

- 三档润色名称调整为轻度、中度、重度
- 轻度仅清理口语、重复、冗余、标点和明显语病；中度在此基础上优化句段与逻辑结构；重度完整优化表达和结构
- 继续严格保留原始事实、约束、示例、变量、路径、代码和专业术语，避免润色时无依据扩写
