...
任务是每天、每周或单次三种类型，包含目标连接、服务器本地时间、开始/执行日期、命令和启用状态。每天/每周任务从开始日期起生效，单次任务在执行日期运行；日期为空时以后端计算出的目标服务器当天为默认值。后台调度器每 15 秒检查一次，以连接记录中的时区计算下一次执行时间，并通过 SSH 在远端执行原始命令。执行输出、退出码、状态和时间会持久化，服务重启后继续读取任务。
