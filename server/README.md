...
独立的 SSH 服务器连接和定时任务管理子项目。它不依赖外层 HannisHub 的 API 或配置，所有运行数据保存在本目录自动生成的 `settings.json` 中。挂载到 HannisHub 时，日常入口由统一 Dashboard 的 `/server/connections` 与 `/server/tasks` 提供；本目录的 `index.html` 和 `/server/` 保留用于兼容。
...
单独运行时默认访问地址为 <http://localhost:8082/server>。在外层 HannisHub 已运行的部署中，`server` 会自动挂载到外层的 `/server` 路由，统一 Dashboard 使用同源 `/server/api` 接口；请优先访问外层地址的 `/server/connections` 或 `/server/tasks`，例如 `https://models.lihan.online/server/connections`。`/server/` 仍可访问旧版页面，不需要额外暴露 8082 端口。可以通过 `SERVER_PORT` 修改独立运行端口，例如 `SERVER_PORT=8081 bash run.sh`。
