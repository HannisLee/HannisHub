...
export function FileManagerPanel() {
  ...
  return (
    <>
      <PageHeader
        kicker="文件管理 / Files"
        title="本机 reproduce 文件浏览"
        description="默认仅暴露 ~/reproduce。目录会在服务端缓存，短时间内重复浏览无需重新扫描磁盘；需要最新状态时可手动同步。"
      ...
      <div className="stack-grid">
        <Card>
          <CardHeader
            title="暴露范围"
            description="每行填写一个顶层文件夹。文件管理、点云扫描和下载都会受到这些目录限制；支持以 ~/ 开头的路径。"
          />
        ...
        <Card className="file-manager-browser">
          <CardHeader
            ...
            actions={
              <div className="file-manager-toolbar">
                <select value={selectedRootIndex} onChange={event => { setSelectedRootIndex(Number(event.target.value)); setPath(""); }} aria-label="顶层文件夹">
          ...
          <nav className="file-manager-breadcrumb" aria-label="目录路径">
            <button type="button" onClick={() => goToDirectory("")}>reproduce</button>
          ...
          {loading ? <LoadingState label="正在读取目录…" /> : !roots.length ? <EmptyState title="没有已暴露的文件夹" detail="先在上方保存至少一个存在的顶层目录。" /> : filteredEntries.length ? (
            <div className="file-manager-list">
              {filteredEntries.map(entry => (
                <div className="file-manager-entry" key={entry.path}>
                  <button className="file-manager-entry-main" type="button" onClick={() => entry.type === "directory" && goToDirectory(entry.path)} disabled={entry.type !== "directory"}>
